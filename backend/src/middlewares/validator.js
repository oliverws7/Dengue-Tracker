...
const validateDengueFocusCreation = [
  body('latitude')
    ...
    .isDecimal()
    .withMessage('Latitude deve ser um número decimal válido')
    .custom((value, { req }) => {
      if (!validateCoordinates(value, req.body.longitude)) {
        throw new Error('Coordenadas geográficas inválidas');
      }
      if (req.body.longitude && !validateBrazilianCoordinates(value, req.body.longitude)) {
        throw new Error('Localização deve estar dentro do território brasileiro');
      }
      return true;
    }),
  ...
  body('longitude')
    ...
    .isDecimal()
    .withMessage('Longitude deve ser um número decimal válido')
    .custom((value, { req }) => {
      if (!validateCoordinates(req.body.latitude, value)) {
        throw new Error('Coordenadas geográficas inválidas');
      }
      if (req.body.latitude && !validateBrazilianCoordinates(req.body.latitude, value)) {
        throw new Error('Localização deve estar dentro do território brasileiro');
      }
      return true;
    }),
  ...
  body('riskLevel')
    .notEmpty()
    .withMessage('Nível de risco é obrigatório')
    .isIn(['baixo_risco', 'medio_risco', 'alto_risco'])
    .withMessage('Nível de risco deve ser: baixo_risco, medio_risco ou alto_risco'),

  body('status')
    .optional()
    .isIn(['monitorando', 'resolvido'])
    .withMessage('Status deve ser: monitorando ou resolvido'),
...
const validateDengueFocusUpdate = [
  body('latitude')
    ...
    .withMessage('Latitude deve ser um número decimal válido')
    .custom((value, { req }) => {
      if (value && req.body.longitude && !validateCoordinates(value, req.body.longitude)) {
        throw new Error('Coordenadas geográficas inválidas');
      }
      if (value && req.body.longitude && !validateBrazilianCoordinates(value, req.body.longitude)) {
        throw new Error('Localização deve estar dentro do território brasileiro');
      }
      return true;
    }),
  ...
  body('longitude')
    ...
    .withMessage('Longitude deve ser um número decimal válido')
    .custom((value, { req }) => {
      if (value && req.body.latitude && !validateCoordinates(req.body.latitude, value)) {
        throw new Error('Coordenadas geográficas inválidas');
      }
      if (value && req.body.latitude && !validateBrazilianCoordinates(req.body.latitude, value)) {
        throw new Error('Localização deve estar dentro do território brasileiro');
      }
      return true;
    }),
  ...
  body('userId')
    .not()
    .exists()
    .withMessage('Não é possível alterar o usuário proprietário do foco'),
...
const validateNearbySearch = [
  ...
  body('radius')
    ...
    .withMessage('Raio deve ser um número entre 0.1 e 100 km'),
